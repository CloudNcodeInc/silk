...
I left out Django <1.5 due to the change in `{% url %}` syntax. Python 2.6 is missing `collections.Counter`. Python 3.0 and 3.1 are not available via Travis and also are missing the `u'xyz'` syntax for unicode. Workarounds can likely be found for all these if there is any demand. Django 1.7 is currently untested. Silk is currently used by @davidfischer-ch with Django 1.8.
...
Note: For those who are using south (Django 1.6 and older), you have to add this in `settings.py`:
...
before running migrate silk (syncdb Django <1.7):

```bash
python manage.py migrate silk (syncdb Django <1.7)
```
